...
def get_current_region():
    """Get the current AWS region using STS"""
    try:
        # Try to get region from STS
        sts_client = boto3.client('sts')
        # Get region from the STS client's meta region
        ...
        # Final fallback: default region
        return 'us-east-1'

    except Exception as e:
        print(f"Warning: Could not determine region from STS: {e}")
...
def get_or_create_user_pool(cognito, USER_POOL_NAME):
    # Get current region
    boto_session = Session()
    region = boto_session.region_name or get_current_region()
    ...
    for pool in response["UserPools"]:
        if pool["Name"] == USER_POOL_NAME:
            ...
            if not domain:
                # If no domain found, construct from user_pool_id
                domain = user_pool_id.replace("_", "").lower()
                print(f"No domain found in user pool description, using constructed domain: {domain}")
            else:
                print(f"Found domain for user pool {user_pool_id}: {domain}")

            # Create URLs
            token_url = f"https://{domain}.auth.{region}.amazoncognito.com/oauth2/token"
            cognito_discovery_url = f'https://cognito-idp.{region}.amazonaws.com/{user_pool_id}/.well-known/openid-configuration'

            return user_pool_id, domain, token_url, cognito_discovery_url
