...
async def process_chat_background(task_id: str, data: dict):
    try:
        ...
        if agent_mode == "Single Agent":
            ...
            print(f"Background task {task_id}: Using Single Agent endpoint: {endpoint_url}")
        else:  # Multi-Agent
            endpoint_url = AGENT_UI_ENDPOINT_URL_2
            print(f"Background task {task_id}: Using Multi-Agent endpoint: {endpoint_url}")
...
async def chat(message, history, agent_mode, request: gr.Request):
    ...
    try:
        ...
        message = f"I'm customer CUST001, {message}"
        task_data = {
            "message": message,
...
def on_gradio_app_load(request: gr.Request):
    ...
    if not request.username:
        request.username = "Sarah Johnson"
    return f"Logout ({request.username})", [gr.ChatMessage(
        role="assistant",
        content=f"Hi {request.username}, I'm your helpful Assitant. Tell me how I can help. "
    )], "Single Agent"  # Default to Single Agent mode

def logout_click(request: gr.Request):
    print(f"logout function")
    #request.session.clear()
    return f"Logout ({request.username})", [], "Single Agent"
